...
async def araid(interaction: discord.Interaction, delay: float = 0.01):
    ...
    raid_message = raid_messages.get("custom", '''>... @everyone

HEELLOOO👋

``STUPID NIGGER OWNERS HEX STARTED
IMAGINE A OWNER AS RETARD AS YOU NOBODY SHOULD BE IN HERE
...
GO TO HEX || SERVER NOW EVERY SINGLE NIGGER IN HERE STOP USING THIS FUCKING SHIT``
━━━━━━━━━━━━┓
...
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
@app_commands.user_install()
async def ghostping(interaction: discord.Interaction, user: discord.User, seconds: float = 0.3, times: int = 3):
...
async def flooduser(interaction: discord.Interaction, user: discord.User, message: str, times: int = 5, delay: float = 0.3):
    ...
    await log_command_use(user=interaction.user, command_name="💎 flooduser", channel=interaction.channel, message=message)
...
async def preset_message(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="⚡ Preset Message", description="Use the buttons below to set or preview your raid message.", color=0xa874d1)
    await interaction.response.send_message(embed=embed, view=view, ephemeral=True)

# Add/remove premium (owner only)
